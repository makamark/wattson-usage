...
bash scripts/package-app.sh            # 产物：release/WattsonNative-2.0.10-arm64.dmg
open release/WattsonNative-2.0.10-arm64.dmg
```

> 命名体系与 Electron 时代的 `Wattson-0.1.x-arm64.dmg`（com.wattson.app）明确区分：
> 原生版为 **WattsonNative**（Bundle ID `com.wattson.native`），版本从 **2.0.10** 起线。
