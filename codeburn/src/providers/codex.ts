...
export function createCodexProvider(
  ...
  return {
    ...
    async discoverSessions(): Promise<SessionSource[]> {
      ...
      if (duplicateHome) return []
      let sources = await discoverSessionsInDir(dir)
      if (scanBoth) {
        const billed = await discoverSessionsInDir(primaryDir)
        sources = [...billed, ...dropOverlappingNestSources(sources, primaryDir)]
      } else if (nestHome) {
        sources = dropOverlappingNestSources(sources, primaryDir)
      }
      // Device roots (devices.json): mirror trees from other machines. Paths
      // are distinct from the local home, so no overlap logic applies.
      for (const device of loadDeviceRoots()) {
        if (!device.codexHome) continue
        const resolved = resolve(device.codexHome)
        if (resolved === resolve(dir) || resolved === resolve(primaryDir)) continue
        sources = sources.concat(await discoverSessionsInDir(resolved))
      }
      return sources
    },
