  ...
  return {
    ...
    async discoverSessions(): Promise<SessionSource[]> {
      ...
      for (const device of loadDeviceRoots()) {
        if (!device.zcodeDb || resolve(device.zcodeDb) === resolve(dbPath)) continue
        for (const source of discover(device.zcodeDb)) {
          if (!seen.has(source.path)) { seen.add(source.path); sources.push(source) }
        }
      }
      return sources
    },
