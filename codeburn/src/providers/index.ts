...
let workbuddyProvider: Provider | null = null
let workbuddyLoadAttempted = false

async function loadWorkbuddy(): Promise<Provider | null> {
  if (workbuddyLoadAttempted) return workbuddyProvider
  workbuddyLoadAttempted = true
  try {
    const { workbuddy } = await import('./workbuddy.js')
...
const lazyProviderNames = ['antigravity', 'forge', 'goose', 'cursor', 'opencode', 'cursor-agent', 'crush', 'warp', 'vercel-gateway', 'zcode', 'zed', 'workbuddy']
...
export async function getAllProviders(): Promise<Provider[]> {
  const [ag, forge, gs, cursor, opencode, cursorAgent, crush, warp, vercelGw, zc, zd, wb] = await Promise.all([
    loadAntigravity(), loadForge(), loadGoose(), loadCursor(), loadOpenCode(), loadCursorAgent(), loadCrush(), loadWarp(), loadVercelGateway(), loadZcode(), loadZed(), loadWorkbuddy(),
  ])
