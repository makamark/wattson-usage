...
export const PROVIDER_ENV_VARS: Record<string, string[]> = {
  ...
  codex: ['CODEX_HOME', 'CODEBURN_DEVICES_FILE'],
  hermes: ['HERMES_HOME'],
  ...
  // devices.json (device-roots.ts) lists per-host mirrors of the zcode db and
  // the workbuddy db; host attribution and workbuddy discovery read it, so a
  // repoint must move those sections' fingerprints. zcode itself reads no env
  // var — this entry exists only for the devices file.
  zcode: ['CODEBURN_DEVICES_FILE'],
  workbuddy: ['WORKBUDDY_HOME', 'CODEBURN_DEVICES_FILE'],
}
