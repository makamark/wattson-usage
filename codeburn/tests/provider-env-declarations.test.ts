...
const FILE_PROVIDERS: Record<string, string[]> = {
  ...
  // Shared device-roots module: its single read (CODEBURN_DEVICES_FILE) drives
  // workbuddy's multi-device discovery and host attribution for zcode/codex
  // sources (hostForSourcePath covers zcodeDb/codexHome/workbuddyDb roots).
  'device-roots.ts': ['zcode', 'codex', 'workbuddy'],
}
